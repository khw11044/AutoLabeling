...
for image_path in images:
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            f = open(xmlfolder+'/'+filename2+".txt", 'w')
            f.write("0" + " " + xmin + " " + ymin + " " + xmax + " " + ymax)
            f.close
            print("succese")
            try:
                shutil.move(PATH_TO_IMAGES+'/'+filename, 'succeedfolder'+'/'+filename)
    ...
    if key == ord('d'):
        print('delete')
        os.remove(xmlfolder+'/'+filename2+".txt")
        print("fail")
        try:
            shutil.move('succeedfolder'+'/'+filename, failimages+'/'+filename)
        except FileNotFoundError :
            cv2.destroyAllWindows()
        cv2.destroyAllWindows()
